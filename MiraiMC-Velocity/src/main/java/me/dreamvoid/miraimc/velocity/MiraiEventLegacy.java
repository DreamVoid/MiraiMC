...
public class MiraiEventLegacy extends MiraiEvent {
	...
	public MiraiEventLegacy(VelocityPlugin plugin) {
		super(plugin);
	...
	public void startListenEvent(){
		...
		// 群
		BotLeaveActiveEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Active.class, event -> server.getEventManager().fire(new MiraiGroupBotLeaveEvent(event)));
		BotLeaveKickEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Kick.class, event -> server.getEventManager().fire(new MiraiGroupBotLeaveEvent(event)));
