...
/**
 * (Velocity) Mirai 核心事件 - 消息 - 主动发送消息后 - 群临时会话消息
 */
public class MiraiGroupTempMessagePostSendEvent extends AbstractMessagePostSendEvent {
    public MiraiGroupTempMessagePostSendEvent(GroupTempMessagePostSendEvent event) {
        ...
        VelocityPlugin.INSTANCE.getServer().getEventManager().fire(new me.dreamvoid.miraimc.velocity.event.MiraiGroupTempMessagePostSendEvent(event));
    }
