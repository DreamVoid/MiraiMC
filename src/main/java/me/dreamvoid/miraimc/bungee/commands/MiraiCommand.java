            ...
            switch (args[0].toLowerCase()){
                ...
                case "logout":{
                    if(sender.hasPermission("miraimc.command.mirai.logout")){
                        if(args.length >= 2) {
                            ...
                            sender.sendMessage(new ComponentBuilder(ChatColor.translateAlternateColorCodes('&',"&a已关闭指定机器人进程！")).create());
                        } else {
                ...
                case "sendgroupnudge":{
                    if(sender.hasPermission("miraimc.command.mirai.sendgroupnudge")){
                        if(args.length >= 3){
                            ...
                            sender.sendMessage(new ComponentBuilder(ChatColor.translateAlternateColorCodes('&',"&c无效的参数！用法: /mirai sendgroupnudge <账号> <群号>")).create());
                        }
                    } else sender.sendMessage(new ComponentBuilder(ChatColor.translateAlternateColorCodes('&',"&c你没有足够的权限执行此命令！")).create());
                    break;
                }
                ...
                case "list":{
                    if(sender.hasPermission("miraimc.command.mirai.list")){
                        sender.sendMessage(new ComponentBuilder(ChatColor.translateAlternateColorCodes('&',"&a当前在线的机器人: ")).create());
                ...
                case "help":{
                    ...
                    sender.sendMessage(new ComponentBuilder(ChatColor.translateAlternateColorCodes('&',"&6/mirai logout <账号>:&r 关闭一个机器人线程")).create());
                    sender.sendMessage(new ComponentBuilder(ChatColor.translateAlternateColorCodes('&',"&6/mirai list:&r 查看当前在线的机器人")).create());
                    ...
                    sender.sendMessage(new ComponentBuilder(ChatColor.translateAlternateColorCodes('&',"&6/mirai sendgroupnudge <账号> <群号>:&r 向指定群发送戳一戳")).create());
