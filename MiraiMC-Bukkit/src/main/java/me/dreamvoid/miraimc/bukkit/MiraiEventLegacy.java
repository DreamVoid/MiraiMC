...
public class MiraiEventLegacy extends MiraiEvent {
	...
	public void startListenEvent(){
		ClassLoader loader = Thread.currentThread().getContextClassLoader(); // since mirai 2.12.0
		...
		// 群
		BotLeaveActiveEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Active.class, event -> Bukkit.getServer().getPluginManager().callEvent(new MiraiGroupBotLeaveEvent(event)));
		BotLeaveKickEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Kick.class, event -> Bukkit.getServer().getPluginManager().callEvent(new MiraiGroupBotLeaveEvent(event)));
