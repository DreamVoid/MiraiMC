...
public class MiraiEventLegacy extends MiraiEvent {
	...
	public void startListenEvent(){
		...
		// 群
		BotLeaveActiveEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Active.class, event -> ProxyServer.getInstance().getPluginManager().callEvent(new MiraiGroupBotLeaveEvent(event)));
		BotLeaveKickEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Kick.class, event -> ProxyServer.getInstance().getPluginManager().callEvent(new MiraiGroupBotLeaveEvent(event)));
