...
public class MiraiEventLegacy extends MiraiEvent{
	...
	public MiraiEventLegacy(NukkitPlugin plugin){
		super(plugin);
		this.plugin = plugin;
	}
	...
	public void startListenEvent(){
		...
		// -- 图片上传完成
		ImageUploadSucceedEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(ImageUploadEvent.Succeed.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiImageUploadSucceedEvent(event)));
		ImageUploadFailedEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(ImageUploadEvent.Failed.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiImageUploadFailedEvent(event)));
		// - 戳一戳
		...
		// 群
		BotLeaveActiveEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Active.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupBotLeaveEvent(event)));
		BotLeaveKickEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotLeaveEvent.Kick.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupBotLeaveEvent(event)));
		BotGroupPermissionChangeEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotGroupPermissionChangeEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupBotPermissionChangeEvent(event)));
		BotMuteEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotMuteEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupBotMuteEvent(event)));
		BotUnmuteEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotUnmuteEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupBotUnmuteEvent(event)));
		BotJoinGroupEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotJoinGroupEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupBotJoinGroupEvent(event)));
		// - 群设置
		...
		// - 群成员
		// -- 成员列表变更
		MemberJoinInviteEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberJoinEvent.Invite.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberJoinEvent(event, event)));
		MemberJoinActiveEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberJoinEvent.Active.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberJoinEvent(event, event)));
		MemberLeaveKickEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberLeaveEvent.Kick.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberLeaveEvent(event, event)));
		MemberLeaveQuitEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberLeaveEvent.Quit.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberLeaveEvent(event, event)));
		MemberJoinRequestEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberJoinRequestEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberJoinRequestEvent(event)));
		BotInvitedJoinGroupRequestEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(BotInvitedJoinGroupRequestEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupBotInvitedJoinGroupRequestEvent(event)));
		// -- 名片和头衔
		MemberCardChangeEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberCardChangeEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberCardChangeEvent(event)));
		MemberSpecialTitleChangeEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberSpecialTitleChangeEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberSpecialTitleChangeEvent(event)));
		// -- 成员权限
		MemberPermissionChangeEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberPermissionChangeEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberPermissionChangeEvent(event)));
		// -- 动作
		MemberMuteEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberMuteEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberMuteEvent(event)));
		MemberUnmuteEventListener = GlobalEventChannel.INSTANCE.subscribeAlways(MemberUnmuteEvent.class, event -> plugin.getServer().getPluginManager().callEvent(new MiraiGroupMemberUnmuteEvent(event)));
